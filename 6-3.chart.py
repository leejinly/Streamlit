...
fig = alt.Chart(fi).mark_circle().encode(x='총매출', y=sel, color='상품', size=sel)
...
tit = st.selectbox(
      '생존자 분석을 위한 항목을 선택하세요.', ['탑승지역별 분석', '객실등급별 분석'] )
if tit == '탑승지역별 분석':
    sel = 'Embarked'
else:
    sel = 'Pclass'
...
with col1:
    ...
    fig.update_layout(font = dict(size = 14), height=400, width=400)
