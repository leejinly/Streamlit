...
option = st.selectbox('호선 선택',df_line_groupby['호선'].unique() )
...
st.write('날짜, 시간대별 인원수', df_sta_drop)
...
st.write('Unpivot', df_sta_melted)

# # '시간' 별 '인원수' 집계 , as_index=False
# # 새로운 데이터 프레임-에 저장 & 확인
df_sta_groupby = df_sta_melted.groupby('시간', as_index=False)['인원수'].sum()
st.write(option, ' 집계 데이터', df_sta_groupby)

# # altair mark_bar  chart + text 그리기
# # 데이터프레임- df_sta_groupby,  x-'시간',  y-'인원수'
chart = alt.Chart(df_sta_groupby, title='시간별 인원수').mark_bar().encode(
         x='시간', y='인원수').properties(width=900, height=350)
text = alt.Chart(df_sta_groupby).mark_text(dx=0, dy=-5, color='black').encode(
